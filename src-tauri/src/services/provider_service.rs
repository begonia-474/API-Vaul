...
pub fn create_provider(
    ...
    let conn = db.conn.lock().unwrap();
    conn.execute(
        "INSERT INTO providers (name, display_name, icon, base_url, api_type, compat_type, category, openai_base_url, anthropic_base_url, description) VALUES (?1, ?2, ?3, '', 'openai', 'openai_compatible', ?4, ?5, ?6, ?7)",
        rusqlite::params![name, display_name, icon, category, openai_base_url, anthropic_base_url, description],
    )
    .map_err(AppError::Database)?;
...
pub fn update_provider_metadata(
    ...
    let conn = db.conn.lock().unwrap();
    conn.execute(
        "UPDATE providers SET openai_base_url = ?1, anthropic_base_url = ?2, description = ?3 WHERE id = ?4",
        rusqlite::params![openai_base_url, anthropic_base_url, description, id],
    )
    .map_err(AppError::Database)?;
